...
def _sizeofints(sizes: list[int]) -> int:
    ...
    # GROMACS uses a 32-byte scratch buffer here. The product of the
    # per-axis sizes can need many more than 5 bytes on large, wide
    # systems (e.g. ~400k atoms), so a 5-element list overflows.
    bytes_arr = [1] + [0] * 31
    num_of_bytes = 1
    for s in sizes:
        tmp = 0
...
def _decompress(f, natoms: int) -> np.ndarray:
    ...
    i = 0
    lfp = 0
    # ``run`` persists across iterations — it is only ever reassigned when a
    # new run-length flag is read (flag == 1). The GROMACS reference decoder
    # keeps it as a function-scope variable for exactly this reason; resetting
    # it to 0 every iteration desyncs the bitstream the moment a run is
    # followed by a flag==0 atom, which then drives ``smallidx`` below
    # FIRSTIDX and divides by zero on ``sizesmall``.
    run = 0
