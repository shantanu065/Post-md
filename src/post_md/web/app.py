...
from post_md.web.runners import RUNNERS, plot_from_data, run_batch, run_info
...
def create_app(workdir: str | Path) -> FastAPI:
    ...
    async def plot_data_endpoint(
        files: list[UploadFile] = File(...),
        labels: str = Form("[]"),
        colors: str = Form("[]"),
        opts: str = Form("{}"),
    ) -> dict[str, Any]:
        """Render a graph from one or more uploaded data files.

        Each ``files[i]`` is a .dat / .xvg / .csv / .txt table; ``labels`` and
        ``colors`` are JSON arrays aligned to it, and ``opts`` is the same JSON
        style object the analysis tabs send. Curves are overlaid on one axes
        and the PNG is written to the workdir, fetchable via /api/result.
        """
        if not files:
            raise HTTPException(400, "Upload at least one data file.")
        try:
            labels_l = json.loads(labels) or []
            colors_l = json.loads(colors) or []
            opts_d = json.loads(opts) or {}
        except json.JSONDecodeError as exc:
            raise HTTPException(400, f"Malformed JSON field: {exc}") from exc
        ...
        for i, uf in enumerate(files):
            ...
            label = ""
            if i < len(labels_l) and labels_l[i]:
                label = str(labels_l[i]).strip()
            if not label:
                label = Path(uf.filename or f"data{i + 1}").stem
            datasets.append((text, label))

        out = _safe_workdir_path(workdir, "uploaded_plot.png")
        loop = asyncio.get_running_loop()
        try:
            result = await loop.run_in_executor(
                None, plot_from_data, datasets, out, opts_d, colors_l,
            )
        except Exception as exc:
            raise HTTPException(
                400, f"Could not plot data: {type(exc).__name__}: {exc}",
            ) from exc
        return result
